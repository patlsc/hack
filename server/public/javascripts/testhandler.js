...
function submitAnswer() {
	...
	if (checked_ans != null) {  //Test if something was checked
		if (String(checked_ans.value) == "qopt" + String(currentQuestionData["correct"])) {
			...
			wrongcontinuebtn.onclick = function () {
				startNewQuestion();
			};
...
function build_result_obj() {
	...
	let final_dist = estimate_final_prob(history)
	let max_prob = 0
	let max_id = -1
	for (const id in final_dist) {
		if (final_dist[id] >= max_prob) {
			max_id = id
			max_prob = final_dist[id]
		}
	}
